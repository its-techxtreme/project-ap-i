-- Skip jobs deferred by per-account upload caps for a rolling 24-hour window.
-- Platform daily limits reset ~24h after uploads (not UTC midnight).
...
comment on function public.claim_next_job(text, integer) is
  'Atomically claim next queued job (skipping rolling-24h daily upload deferrals), or reclaim an expired lock.';
